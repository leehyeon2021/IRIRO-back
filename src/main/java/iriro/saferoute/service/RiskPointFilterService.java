...
public class RiskPointFilterService {
    ...
    // 우회 경유지 리스트 반환하는 함수
    public List<DetourWayPointDto> getDetourWayPoint(List<RoutePointDto> routePoints,List<RoutePointDto> firstInDangerPoints){
        ...
        if(!secondInDangerPoints.isEmpty()){ // 경로 상에 위험지역이 있으면...
            for(RoutePointDto riskPoint : secondInDangerPoints){ // 경로에 걸치는 위험지역마다 우회경로 포인트 생성
                ...
                if(sequence == 1) continue; // 첫번째 위치일 경우 무시
